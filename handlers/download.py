    ...
    if is_playlist(url):
        ...
        try:
            ...
            await status.edit_text(
                f"Плейлист найден!\n\n{title}\nВидео: {count} (макс. {settings.PLAYLIST_LIMIT})\n\nВыбери что скачать:",
...
async def process_single_download(callback: CallbackQuery, state: FSMContext):
    ...
    if not url:
        await callback.message.edit_text("Ссылка потерялась. Отправь её заново!")
        await state.clear()
        return
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            if callback.data.startswith("video_"):
                ...
                if os.path.getsize(filepath) > settings.MAX_FILE_SIZE:
                    await callback.message.edit_text("Файл превышает лимит 50 МБ. Попробуй качество пониже.")
                    return
                await callback.message.edit_text("Отправляю видео...")
                with open(filepath, "rb") as f:
                    await callback.message.answer_video(video=f, caption=title)
                await log_download(user_id, "video", "success")
            else:
                filepath, title = await download_audio(url, tmpdir)
                if os.path.getsize(filepath) > settings.MAX_FILE_SIZE:
                    await callback.message.edit_text("Файл превышает лимит 50 МБ.")
                    return
                await callback.message.edit_text("Отправляю аудио...")
                with open(filepath, "rb") as f:
                    await callback.message.answer_audio(audio=f, title=title, caption=title)
